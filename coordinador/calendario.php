...
<head>
<meta charset="UTF-8">
<title>Calendario del Coordinador</title>
...
<main class="main">
    <section class="card">
        <h3>Calendario general reservas</h3>
...
function showDayAvailability(dateKey) {
    ...
    espaciosConReserva.forEach(espacio => {
        ...
        if (huecos.length > 0) {
            huecos.forEach(h => {
                const urlReserva = `nueva_reserva.php?fecha=${encodeURIComponent(dateKey)}&espacio_id=${encodeURIComponent(espacio.id)}&hora_inicio=${encodeURIComponent(h[0].substring(0,5))}&hora_fin=${encodeURIComponent(h[1].substring(0,5))}`;
                html += `
                    <div class="libre-item">
                        <p><strong>Libre:</strong> ${formatearHora(h[0])} - ${formatearHora(h[1])}</p>
                        <div class="acciones">
                            <a class="btn" href="${urlReserva}">Reservar este horario</a>
                        </div>
                    </div>
                `;
            });
