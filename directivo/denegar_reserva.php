...
if (($reserva['estado'] ?? '') !== 'pendiente') {
    $_SESSION['error_reserva'] = "Solo se pueden denegar reservas en estado pendiente.";
    header("Location: reservas.php");
    exit();
}
...
if (!$stmt) {
    $_SESSION['error_reserva'] = "No se pudo preparar la denegación de la reserva.";
    header("Location: reservas.php");
    exit();
}
...
if ($stmt->execute() && $stmt->affected_rows > 0) {
    $_SESSION['ok_reserva'] = "La reserva fue denegada correctamente.";
} else {
    $_SESSION['error_reserva'] = "No se pudo denegar la reserva.";
}
